...
async fn main_search(
    ...
    let birthdate_start: String = search
        .get("birthdateStart")
        .unwrap_or(&String::from(""))
        .clone();
    let birthdate_end: String = search
        .get("birthdateEnd")
        .unwrap_or(&String::from(""))
        .clone();
    ...
    if !cleaned_search.is_empty() {
        conditions.push("MATCH(WHOLE_NAME,TRUE_NAME,NAME_ALIAS_FIRST,NAME_ALIAS_MIDDLE, NAME_ALIAS_LAST) AGAINST(? IN BOOLEAN MODE)".to_string());
    ...
    if !datasets.is_empty() {
        ...
        let datasets_conditions = datasets_vec
            .iter()
            .map(|&dataset| {
            ...
            .filter(|condition| !condition.is_empty()) // Empty conditions should be filtered out
            .collect::<Vec<_>>();

        if !datasets_conditions.is_empty() {
            conditions.push(format!("({})", datasets_conditions.join(" OR "))); // Use OR operator here
        }
    }
    ...
    let total_count = match count_result {
        ...
        Err(e) => {
            ...
            return HttpResponse::InternalServerError()
                .json(json!({ "message": "Can't find data" }));
        }
    };
...
async fn business_main_search(
    ...
    if !match_against_fields.is_empty() {
        ...
        let values = match_against_values
            .iter()
            .map(|value| {
